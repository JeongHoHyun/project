					...
					<form class="d-flex" action="/member/list" method="get">
						...
						<div class="input-group">
							...
							<select class="form-select" name="type">
								<option${type eq null ? 'selected':'' } }>선택</option>
								...
								<option value="en" ${typed eq 'en' ? 'selected':'' }>Email
									or NickName</option>
								<option value="eg" ${typed eq 'eg' ? 'selected':'' }>Email
									or grade</option>
								<option value="ng" ${typed eq 'gn' ? 'selected':'' }>NickName
									or Grade</option>
				...
				<tbody>
					<c:forEach items="${list }" var="mvo">
						<tr>
							<td>${mvo.email }</td>
							<td><a
								href="/member/detail?email=${mvo.email }&pageNo=${pgn.pgvo.pageNo }&qty=${pgn.pgvo.qty}">${mvo.nickName }</a></td>
							<td>${mvo.regAt}</td>
							<td>${mvo.lastLogin }</td>
							<td>${mvo.grade }</td>
						</tr>
			...
			<ul class="pagination justify-content-center">
				<c:if test="${pgn.prev }">
					<li class="page-item"><a class="page-link"
						href="/member/list?pageNo=${pgn.startPage-1 }&qty=${pgn.pgvo.qty}&type=${pgn.pgvo.type}&kw=${pgn.pgvo.kw}">Prev</a></li>
				</c:if>
				<c:forEach begin="${pgn.startPage }" end="${pgn.endPage }" var="i">
					<li class="page-item ${pgn.pgvo.pageNo == i ? 'active' : '' }"><a
						class="page-link"
						href="/member/list?pageNo=${i }&qty=${pgn.pgvo.qty}&type=${pgn.pgvo.type}&kw=${pgn.pgvo.kw}">${i }</a></li>
				</c:forEach>
				<c:if test="${pgn.next }">
					<li class="page-item"><a class="page-link"
						href="/member/list?pageNo=${pgn.endPage+1 }&qty=${pgn.pgvo.qty}&type=${pgn.pgvo.type}&kw=${pgn.pgvo.kw}">Next</a></li>
				</c:if>
			</ul>
		</c:when>
		<c:otherwise>
